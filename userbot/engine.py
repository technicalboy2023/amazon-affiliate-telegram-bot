...
class UserbotEngine:
    ...
    async def _on_new_message(self, event: events.NewMessage.Event) -> None:
        ...
        affiliate_tag = self.db.get_affiliate_tag()
        ...
        if found_asins:
            for asin in found_asins:
                if self.db.check_and_mark_dedup(asin, source_db_id):
                    is_duplicate = True
                    break
        else:
            if raw_text and self.db.check_and_mark_dedup(raw_text, source_db_id):
                is_duplicate = True
