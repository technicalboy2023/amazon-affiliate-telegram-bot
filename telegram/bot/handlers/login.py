...
# start over (seconds).
_LOGIN_TIMEOUT = 120  # 2 minutes – matches Telegram OTP expiry
...
async def process_otp(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        # Code expired – auto-resend triggers Telegram's security "code shared" block!
        # User must start fresh with /login
        _cleanup_login(user_id)
        await state.clear()
        await message.answer(
            "⌛ The confirmation code has expired.\n\n"
            "⚠️ For security, please start a fresh login with /login\n"
            "and enter the code as soon as you receive it."
        )
