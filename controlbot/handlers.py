...
async def set_tag(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await _admin_only(update):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Usage: /set_tag <affiliate_tag>")
        return
    tag = context.args[0].strip()
    db: Database = context.bot_data["db"]
    db.set_affiliate_tag(tag)
    await update.message.reply_text(
        f"✅ <b>Amazon Affiliate Tag set to:</b> <code>{_escape_html(tag)}</code>", parse_mode="HTML"
    )


async def clear_tag(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await _admin_only(update):
        return
    db: Database = context.bot_data["db"]
    db.clear_affiliate_tag()
    await update.message.reply_text("🗑️ Affiliate tag cleared.")
...
async def set_domain(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"✅ <b>Amazon Default Domain set to:</b> <code>{_escape_html(domain)}</code>", parse_mode="HTML"
    )
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    affiliate_tag = db.get_affiliate_tag() or "(None)"
    amazon_domain = db.get_amazon_domain() or "amazon.in"
    ...
    msg = (
        ...
        f"🔗 <b>Affiliate Tag:</b> <code>{_escape_html(affiliate_tag)}</code>\n"
        f"🌐 <b>Amazon Domain:</b> <code>{_escape_html(amazon_domain)}</code>\n\n"
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await _admin_only(update):
        return
    welcome = (
        "👋 <b>Welcome to Amazon Affiliate Bot Admin!</b>\n\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        "📘 <b>PRO USER MANUAL — Amazon Affiliate Auto-Forwarding Bot</b>\n\n"
        "<i>Use this guide to master your bot's features. Note: The bot uses a 1-hour cross-channel duplicate filter automatically.</i>\n\n"
        "🔗 <b>AMAZON AFFILIATE SETTINGS</b>\n"
        "▪️ <code>/set_tag &lt;tag&gt;</code> — Set your Amazon affiliate tag (e.g. <i>my_tag-21</i>)\n"
        "▪️ <code>/clear_tag</code> — Remove your affiliate tag\n"
        "▪️ <code>/set_domain &lt;domain&gt;</code> — Set default amazon domain (e.g. <i>amazon.in</i>)\n\n"
        ...
        "▪️ <code>/add_replace &lt;old&gt; &lt;new&gt;</code> — Auto-replace words (e.g., <i>/add_replace Flipkart Amazon</i>)\n"
